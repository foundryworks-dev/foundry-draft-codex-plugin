...
uninstall() {
    ...
    rm -rf "$INSTALL_ROOT"
    info "removed installed files ($INSTALL_ROOT, prompts, broker client)"
...
main() {
    ...
    info "installing prompts + broker client ..."
    mkdir -p "$CODEX_HOME/prompts" "$CODEX_HOME/scripts"
    cp "$dest"/prompts/*.md "$CODEX_HOME/prompts/"
