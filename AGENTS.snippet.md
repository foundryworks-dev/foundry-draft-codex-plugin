...
`$foundry-work` explicitly when you want it.
...
`$foundry-queue`. To start working tickets from it, run
`$foundry-work`, or `$foundry-watch` to keep working as
new tickets arrive. To re-pull the latest workflow instructions
mid-session, run `$foundry-refresh`. If `FOUNDRY_API_KEY` is *not*
set but a Foundry Agent Registry daemon is running, run
`$foundry-agents` to pick and claim a key from the registry
